...
exports.compareWishlists = async (req, res) => {
	try {
		...
		if (findUserWishlist.length === 0) throw { message: 'The user has no movies on the  wishlist' };

		if (findFriendWishlist.length === 0) throw { message: 'Your friend has no movies on the  wishlist' };

		// Getting two arrays with the movieIds from the user's and friend's wishlist
		const userMovieIdArray = findUserWishlist.map(item => item['movieId']);

		const friendMovieIdArray = findFriendWishlist.map(item => item['movieId']);

		// Finding the movies in common in both wishlists
		const movieIdInBothArrays = userMovieIdArray.filter(item => friendMovieIdArray.includes(item));
