	...
	return (
		<div className='friends-page'>
			...
			<form onSubmit={searchForUser} className='form-search-user'>
				...
				<input required type='text' id='header-search' placeholder='Find a new friend' value={keyword} onChange={e => setKeyword(e.target.value)} />
				<button type='submit'>Search</button>
			</form>

			{/* ALERT MESSAGE */}
			{isError ? <div className='alert-message'>{errorMessage}</div> : null}

			{/* DISPLAY SEARCH RESULTS */}
			{userFounded ? (
				<div className='friends-page-display-box'>
					<h2>Search Results</h2>
					<Friend searchOrFriends={searchedUser} isFriend={isAlreadyFriend} />
			...
			{friends.length !== 0 && !userFounded ? (
				<div className='friends-page-display-box'>
					<h2>My Friends</h2>
					<Friend searchOrFriends={friends} isFriend={isAlreadyFriend} />
				</div>
			) : null}
